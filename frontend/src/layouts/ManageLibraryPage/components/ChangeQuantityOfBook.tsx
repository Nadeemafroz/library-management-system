...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props) => {
    ...
    async function increaseQuantity() {
        ...
        setIsLoading(true);
        try {
            const url = `http://13.127.63.213:8080/api/admin/secure/increase/book/quantity?bookId=${props.book?.id}`;
    ...
    async function decreaseQuantity() {
        ...
        setIsLoading(true);
        try {
            const url = `http://13.127.63.213:8080/api/admin/secure/decrease/book/quantity?bookId=${props.book.id}`;
            const requestOptions = {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async function deleteBook() {
        ...
        setIsLoading(true);
        try {
            const url = `http://13.127.63.213:8080/api/admin/secure/delete/book/?bookId=${props.book?.id}`;
            const requestOptions = {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
